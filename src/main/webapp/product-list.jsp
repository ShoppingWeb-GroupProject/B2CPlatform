...
<body>
                ...
                <c:forEach var="product" items="${showProducts}">
                    <div class="grid-row ${action == 'showForSeller' ? 'grid-ForSeller' : 'grid-all'}">
                        ...
                        <c:if test="${action=='showForSeller'}">
                            <div>
                                <a
                                    href="ProductController?action=modify&productId=${product.id}"><button>編輯</button></a>
                                <a
                                    href="ProductController?action=delete&productId=${product.id}"><button>刪除</button></a>
                            </div>
                        </c:if>
            ...
            <c:if test="${action == 'modify'}">
                ...
                <form action="ProductController" method="post">
                    ...
                    <div class="form-grid">
                        <label for="name">商品名稱：</label> <input type="text" id="name" name="name" value="${product.name}"
                            required /> <label for="price">價格：</label>
                        <input type="number" id="price" name="price" step="0.01" value="${product.price}" required />
                        <label for="stock">庫存：</label>
                        <input type="number" id="stock" name="stock" value="${product.stock}" required /> <label
                            for="description">商品描述：</label>
                        <textarea id="description" name="description" rows="4">${product.description}</textarea>

                        <label for="categoryId">分類：</label> <select id="categoryId" name="categoryId" required>
